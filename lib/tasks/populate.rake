  ...
  namespace :populate do
    ...
    namespace :mail_template do
      ...
      task :user_application_confirmation => :environment do
        m = MailTemplate.where(:title => 'user_application_confirmation').first
        m.destroy if m
      ...
      task :user_invitation => :environment do
        m = MailTemplate.where(:title => 'user_invitation').first
        m.destroy if m
      ...
      task :teacher_invitation => :environment do
        m = MailTemplate.where(:title => 'teacher_invitation').first
        m.destroy if m
...
def create_users(count)
  MiscHelpers.disable_perform_deliveries do
