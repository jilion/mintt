require 'spec_helper'

describe Admin::MessagesController do

  %w(update).each do |action|
    it "should not save non-registered attributes on #{action} action" do
      id = "4b8e9831d9e93410a7000001"
      Message.should_receive(:find).with(id).and_return(Factory.create(:message))
      post action, :message => Factory.attributes_for(:message).merge({:not_registered_key => 'foo'}), :id => id
      params.include?(:not_registered_key).should_not be_true
    end
  end
  
end
